// steamgpt: CJS mirror of index.mjs - typed client for the free Steam data API
// at steamgpt.net. No keys; {id} accepts steamid64, STEAM_1:0:x, [U:1:x],
...
const summary  = ( player, opts = {} ) => requestJson( '/summary/' + id( player ) + '.json' + buildQuery( opts ) )
const profile  = ( player )           => requestJson( '/profile/' + id( player ) + '.json' )
const bans     = ( player )           => requestJson( '/bans/' + id( player ) + '.json' )
const faceit   = ( player )           => requestJson( '/faceit/' + id( player ) + '.json' )
const friends  = ( player, opts = {} ) => requestJson( '/friends/' + id( player ) + '.json' + buildQuery( opts ) )
const identity = ( player )           => requestJson( '/identity/' + id( player ) + '.json' )
const compare  = ( player_a, player_b ) => requestJson( '/compare/' + id( player_a ) + '/' + id( player_b ) + '.json' )

// batch: up to 100 steamid64 STRINGS per request - always cheaper than per-player calls
const batch = ( steamids64, opts = {} ) => requestJson( '/batch', {
    "method": 'POST',
    "body":   { "ids": steamids64.map( ( value ) => String( value ) ), ...( Array.isArray( opts?.include ) ? {"include": opts.include} : {} ) },
})

module.exports = { summary, profile, bans, faceit, friends, identity, compare, batch }
